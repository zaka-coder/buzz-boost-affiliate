@extends('layouts.seller', ['title' => 'Messages'])
@section('css')
    <style>
        ...
        <div class="connected__user w-20 py-3 h-100">
            <ul class="m-0 p-0">
                <li class="position-relative chat_search_wrapper">
                    ...
                    <input type="search" name="" id="" class="w-90" placeholder="search">
                </li>
                @forelse ($receiversData as $receiver)
                    <li class="my-4">
                        <a href="{{ route('chats.show', ['role' => 'seller', 'receiver_id' => $receiver->id]) }}"
                            class="connected_user_profile text-dark">
                            <img src="{{ asset('assets/buyer-assets/user-icon.png') }}" alt=""
                                class="connected_user-img">
                            <p class="connected_username" style="overflow: hidden">{{ strtolower(str_replace(' ', '', $receiver->name)) }}</p>
                        </a>
                    </li>
                @empty
            ...
            <div class="single__connected__user w-80 h-100 ">
                <div class="connected_user_info w-100 h-15 bg-white d-flex align-items-center gap-2 ps-3">
                    ...
                    <p class="connected_username"><span>Chat with</span><span class="fs-4">
                            {{ strtolower(str_replace(' ', '', $recepient->name)) }}</span></p>
                </div>
                    ...
                    @else
                        @foreach ($messages as $message)
                            <div class="">
                                ...
                                {{-- recepient messege --}}
                                <div class="my-3 w-80">
                                    <p class="connected_username">{{ strtolower(str_replace(' ', '', $recepient->name)) }}</p>
                                    <div class="messege">
                                        {{ $message->message }}
                ...
                <div class="w-100 conversation_wrapper h-10">
                    <form id="reply_form" action="{{ route('chats.reply', ['role' => 'seller', 'receiver_id' => $recepient->id]) }}" class="h-100 w-95 m-auto position-relative" method="post">
                        @csrf
